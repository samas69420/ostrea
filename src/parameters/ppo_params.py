...
params = Params(# environment/general training parameters 
                ...
                PPO_EPS = 0.1,                   # threshold for the ratio function
                ...
                SQUASH_ACTION = True,            # limit the actions in the range [-1,1]
                NORMALIZE_OBSERVATIONS = False,  # normalize observation (linear scaling normalization)
                DEPLOY_STOCHASTIC_POLICY = False,# test and evaluate the stochastic policy instead of its deterministic version
                ENCODER_TYPE = "cnn",            # vit/cnn encoder for 3d input
                MIN_COV = 1e-2,                  # minimum value allowed for diagonal cov matrix
                EPOCHS = 10,
                BATCH_SIZE = 128,                # for now these batches are made only along the time dimension
                LR = 1e-3,
                NUMERICAL_EPSILON = 1e-7,        # value for numerical stability
                BETA = 1e-3,                     # weight used for entropy
